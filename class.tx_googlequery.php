...
class tx_googlequery extends tx_tesseract_providerbase {
	...
	public function getData( ) {
		...
		if ( is_array( $GLOBALS[ 'TYPO3_CONF_VARS' ][ 'EXTCONF' ][ $this->extKey ][ 'postProcessDataStructure' ] ) ) {
			foreach (
				$GLOBALS[ 'TYPO3_CONF_VARS' ][ 'EXTCONF' ][ $this->extKey ][ 'postProcessDataStructure'
				] as $className ) {
				...
				if ( $start_from < $total ) {
					if ( $xml->RES->R ) {
						...
						foreach ( $xml->RES->R as $infos ) {
							...
							if ( $att[ 'MIME' ] ) {
								$results[ $res ][ 'googleInfos$mimetype'
								] = tx_googlequery_mimetypes::getSmallMimeTypeName( (string) $att[ 'MIME' ] );
							}
							else {
								$results[ $res ][ 'googleInfos$mimetype'
								] = tx_googlequery_mimetypes::getSmallMimeTypeName( 'text/html' );
							}
							...
							if ( $infos->HAS->C ) {
								...
								$results[ $res ][ 'googleInfos$cachedurl'
								] = (string) $this->gquery_serverurl . "?q=cache:" .
								    $cachedAtt[ 'CID' ] . ":" . (string) $infos->UD . '+' . $cacheParams[ 'q' ];
								unset( $cacheParams[ 'q' ] );
								foreach ( $cacheParams as $Key => $Value )
									$results[ $res ][ 'googleInfos$cachedurl' ] .= '&' . $Key . '=' . $Value;
								$results[ $res ][ 'googleInfos$cachedurl'
								] .= '&proxystylesheet=' . $this->providerData[ 'client_frontend' ];
...
if ( defined( 'TYPO3_MODE' ) &&
     $TYPO3_CONF_VARS[ TYPO3_MODE ][ 'XCLASS' ][ 'ext/googlequery/class.tx_googlequery.php' ] ) {
