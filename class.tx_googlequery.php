...
*
* $Id: class.tx_googlequery.php 216 2009-08-19 14:01:50Z rpresedo $
***************************************************************/
/**
 ...
 *
 *
 *   62: class tx_googlequery extends tx_basecontroller_providerbase
 ...
 */
class tx_googlequery extends tx_basecontroller_providerbase {
     ...
     * @return	void
     */
    public function initialise() { // TO CHECK
    ...
    // Prepare the limit and offset parameters
        ...
        //if (isset($items_num) && $items_num !== 0) $this->gquery_Parser->limit_numitems = $items_num;
     ...
     *  @return	array
     */
    protected function buildDataStructure() { // OK
     ...
     */
    protected function prepareFullStructure($res) { // TO EDIT
...
//die();
        ...
        if (!empty($this->providerData['cache_duration'])) {
            $fields = array(
                ...
                'query_uri' => tx_basecontroller_parser::$extraData['query_uri']
            );
     ...
     */
    protected function getCachedStructure() { // OK

    // Cleaning cache deleting old infos
        $GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_googlequery_cache',"tstamp < '".time()."'");
        ...
        if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) == 0) {
            ...
            tx_basecontroller_parser::setExtraData(array('query_uri'=>$row['query_uri']));
     ...
     */
    protected function calculateCacheHash(array $parameters) { // OK
    ...
    // This makes it possible to vary the cache as a function of the idList provided by a secondary provider
        ...
        $hash = tx_basecontroller_utilities::calculateFilterCacheHash($cacheParameters);
        return $hash;
    }
     ...
     * @return	void
     */
    protected function loadQuery() { // OK
     ...
     */
    public function getMainTableName() { // OK
     ...
     * @return	void
     */
    protected function __setConfigParams() { // OK
     ...
     * @return void
     */
    protected function __setSelectedFields() { // OK
     ...
     * @return	array
     */
    protected function __getXmlStructure() { // OK
        $query = tx_basecontroller_parser::evaluateString($this->gquery_query);

        if ($output = file_get_contents($query)) {
