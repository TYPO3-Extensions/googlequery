...
_bsn.AutoSuggest = function (fldID, param) {
	...
	if (!this.oP.timeout)									this.oP.timeout = 50000;
	if (!this.oP.delay)										this.oP.delay = 500;
	if (!this.oP.offsety)									this.oP.offsety = -5;
	if (!this.oP.shownoresults)								this.oP.shownoresults = false;
	if (!this.oP.noresults)									this.oP.noresults = "No suggestion!";
	...
	if (this.oP.json) {
		if ( req.responseText != '' ) {
			...
			for (var i = 0; i < jsondata.results.length; i++) {
				this.aSuggestions.push({ 'name':jsondata.results[i].name, 'type':jsondata.results[i].type});
			}
		}
	}
	else {
		...
		for (var i = 0; i < results.length; i++) {
			if (results[i].hasChildNodes())
				this.aSuggestions.push({ 'name':results[i].getAttribute('name'), 'type':results[i].getAttribute('type') });
		}
...
_bsn.AutoSuggest.prototype.createList = function(arr) {
	...
	if (arr.length == 0 && this.oP.shownoresults == false) {
		this.clearSuggestions();
		return true;
	}
	...
	// no results
	//
	if (arr.length == 0 && this.oP.shownoresults == true) {
